...
CREATE TABLE prices(
    ...
    p_building MEDIUMINT UNSIGNED NOT NULL,
    p_daily BOOLEAN NOT NULL DEFAULT TRUE,
    p_start_sec MEDIUMINT UNSIGNED NOT NULL,
    p_end_sec MEDIUMINT UNSIGNED NOT NULL,
    p_rub MEDIUMINT UNSIGNED NOT NULL,
    p_discount_per TINYINT UNSIGNED NOT NULL DEFAULT 0,
    p_updated_at DATETIME NOT NULL DEFAULT CURRENT_TIMESTAMP,
    PRIMARY KEY (p_id),
    FOREIGN KEY (p_building) REFERENCES buildings(bu_id)
        ON DELETE CASCADE
        ON UPDATE CASCADE
) ENGINE=InnoDB CHARACTER SET=UTF8;
